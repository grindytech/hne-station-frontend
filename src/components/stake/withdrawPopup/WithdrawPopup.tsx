...
const WithdrawPopup: React.FC<Props> = ({ isOpen, withdrawableAmount, onClose, onSuccess }) => {
  ...
  const { mutate, isLoading } = useMutation(pendingWithdrawHE, {
    onSuccess: () => {
      toast.success(
        `Submit withdraw ${formatNumber(Number(value))} ${
    ...
    <>
      ...
      >
        ...
        <ModalContent>
          ...
          <ModalBody>
            <Text>
              Please wait 3 days before withdrawing {configs.TOKEN_SYMBOL}.
              After that period, you can withdraw {configs.TOKEN_SYMBOL} to your
              wallet.
            </Text>
          </ModalBody>
          <ModalFooter justifyContent="center">
